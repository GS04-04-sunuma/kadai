...
<div class="bookdata">
    ...
    <p><?php echo $this->Html->link('TOP', '/'); ?></p>

    <h1 class="booktitle"><?php echo h($book['BookData']['title']); ?></h1>
    <br>

    <div class="bookInfo">
        <p><small>作者:<?php echo $book['BookData']['author']; ?></small></p>
        <p><small>出版社:<?php echo $book['BookData']['manufacturer']; ?></small></p>
        <?php echo $this->Html->image($book['BookData']['imgL'], array('height'=>'500'));?>
    ...
    <?php
        ...
        echo $this->Form->create('BookEvaluation', array('onsubmit'=>'return confirmBox();'));
...
<script>
    ...
    function confirmBox() {
        if (window.confirm('評価を登録します。')) {
            onClick = "loading()";
            location.href = "<?php echo $this->Html->url('/view', array('url' => array('action' => 'add', $book['BookData']['id']))); ?>";
            $.blockUI();
        } else {
